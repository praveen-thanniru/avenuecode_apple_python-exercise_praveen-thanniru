...
class TestSelect:
    ...
    def test_return_dtype(self):
        assert_equal(
            select(self.conditions, self.choices, 1j).dtype, np.complex128
        )
...
class TestDelete:
    ...
    def _check_inverse_of_slicing(self, indices):
        ...
        assert_array_equal(
            setxor1d(a_del, self.a[indices,]), self.a, err_msg=msg
        )
...
class TestVectorize:
    ...
    def test_datetime_conversion(self):
        ...
        assert_array_equal(
            np.vectorize(lambda x: x, signature="(i)->(j)", otypes=[otype])(
                arr
            ),
            arr,
        )
...
class TestPercentile:
    ...
    def test_basic(self):
        ...
        assert_equal(
            np.percentile(
                x, 0, method="inverted_cdf", weights=np.ones_like(x)
            ),
            np.nan,
        )
...
class TestQuantile:
    ...
    def test_quantile_add_and_multiply_constant(self, weights, method, alpha):
        ...
        # 1
        assert_allclose(
            np.quantile(c + y, alpha, method=method, weights=w), c + q
        )
        # 2
        assert_allclose(
            np.quantile(c * y, alpha, method=method, weights=w), c * q
