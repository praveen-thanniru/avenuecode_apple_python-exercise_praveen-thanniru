...
def readfortrancode(ffile, dowithline=show, istop=1):
    ...
    while True:
        ...
        if fin.isfirstline():
            ...
            if Path(
                currentfilename
            ).suffix.lower() in COMMON_FIXED_EXTENSIONS and not (
...
subroutinepattern = (
    re.compile(
        beforethisafter % (r"[a-z\s]*?", "subroutine", "subroutine", ".*"),
        re.I,
    ),
    "begin",
)
...
endpattern = (
    re.compile(beforethisafter % ("", groupends, groupends, ".*"), re.I),
    "end",
)
...
dimensionpattern = (
    re.compile(
        beforethisafter % ("", "dimension|virtual", "dimension|virtual", ".*"),
        re.I,
    ),
    "dimension",
)
...
datapattern = (
    re.compile(beforethisafter % ("", "data", "data", ".*"), re.I),
    "data",
)
callpattern = (
    re.compile(beforethisafter % ("", "call", "call", ".*"), re.I),
    "call",
)
entrypattern = (
    re.compile(beforethisafter % ("", "entry", "entry", ".*"), re.I),
    "entry",
)
callfunpattern = (
...
usepattern = (
    re.compile(beforethisafter % ("", "use", "use", ".*"), re.I),
    "use",
)
containspattern = (
...
def crackline(line, reset=0):
    ...
    if has_semicolon and not (
        f2pyenhancementspattern[0].match(line)
...
def appenddecl(decl, decl2, force=1):
    ...
    for k in list(decl2.keys()):
        if k == "typespec":
            ...
            raise Exception(
                "appenddecl: Unknown variable definition key: " + str(k)
            )
    return decl


selectpattern = re.compile(
    r"\s*(?P<this>(@\(@.*?@\)@|\*[\d*]+|\*\s*@\(@.*?@\)@|))(?P<after>.*)\Z",
    re.I,
)
...
def analyzeline(m, case, line):
    ...
    if case in ["begin", "call", "callfun"]:
        ...
        if args:
            args = rmbadname(
                [x.strip() for x in markoutercomma(args).split("@,@")]
            )
        else:
            args = []
        if "" in args:
        ...
        if block in ["function", "subroutine"]:  # set global attributes
            ...
            if bindcline:
                ...
                if bindcdat:
                    ...
                    groupcache[groupcounter]["bindlang"][name]["lang"] = (
                        bindcdat.group("lang")
                    )
                ...
                if _intentcallbackpattern.match(ap):
                    if k not in groupcache[groupcounter]["args"]:
                        if groupcounter > 1:
                            if (
                                "__user__"
                                not in groupcache[groupcounter - 2]["name"]
                            ):
                                outmess(
...
def updatevars(typespec, selector, attrspec, entitydecl):
    ...
    for e in el:
        for e1 in [
            x.strip()
            for x in markoutercomma(
                removespaces(markinnerspaces(e)), comma=" "
            ).split("@ @")
        ]:
            if e1:
...
def postcrack2(block, tab="", param_map=None):
    ...
    if isinstance(block, list):
        ret = [
            postcrack2(g, tab=tab + "\t", param_map=param_map) for g in block
        ]
        return ret
    setmesstext(block)
    outmess("%sBlock: %s\n" % (tab, block["name"]), 0)
...
def postcrack(block, args=None, tab=""):
    ...
    if not isinstance(block, dict) and "block" not in block:
        raise Exception(
            "postcrack: Expected block dictionary instead of " + str(block)
        )
...
def analyzevars(block):
    ...
    for n in svars:
        ...
        if "typespec" not in vars[n]:
            if not (
                "attrspec" in vars[n] and "external" in vars[n]["attrspec"]
            ):
                if implicitrules:
                    ln0 = n[0].lower()
                    for k in list(implicitrules[ln0].keys()):
                        if (
                            k == "typespec"
                            and implicitrules[ln0][k] == "undefined"
                        ):
                            continue
                        if k not in vars[n]:
            ...
            for a in attr:
                ...
                if intent:
                    if "intent" not in vars[n]:
                        vars[n]["intent"] = []
                    for c in [
                        x.strip() for x in markoutercomma(intent).split("@,@")
                    ]:
                ...
                if depend is not None:
                    ...
                    for c in rmbadname(
                        [
                            x.strip()
                            for x in markoutercomma(depend).split("@,@")
                        ]
                    ):
                ...
                if check is not None:
                    if "check" not in vars[n]:
                        vars[n]["check"] = []
                    for c in [
                        x.strip() for x in markoutercomma(check).split("@,@")
                    ]:
        ...
        if "check" not in vars[n] and "args" in block and n in block["args"]:
            ...
            n_checks = []
            n_is_input = l_or(isintent_in, isintent_inout, isintent_inplace)(
                vars[n]
            )
            if isarray(vars[n]):  # n is array
                    ...
                    if coeffs_and_deps is None:
                        ...
                        for v, (solver, deps) in coeffs_and_deps.items():
                            ...
                            if solver is not None and v not in all_deps:
                                ...
                                is_required = False
                                init = solver(
                                    symbolic.as_symbol(f"shape({n}, {i})")
                                )
                                init = init.tostring(
                            ...
                            if not (
                                "optional" in v_attr or "required" in v_attr
                            ):
                                v_attr.append(
                ...
                if "charselector" in vars[n]:
                    if "*" in vars[n]["charselector"]:
                        length = _eval_length(
                            vars[n]["charselector"]["*"], params
                        )
                        vars[n]["charselector"]["*"] = length
                    elif "len" in vars[n]["charselector"]:
                        length = _eval_length(
                            vars[n]["charselector"]["len"], params
                        )
...
def param_eval(v, g_params, params, dimspec=None):
    ...
    if len(dimrange) == 1:
        ...
        raise ValueError(
            f"param_eval: multidimensional array parameters "
...
def expr2name(a, block, args=[]):
    ...
    if a_is_expr:
        ...
        if (
            "externals" in block
            and orig_a in block["externals"] + block["interfaced"]
        ):
...
def determineexprtype(expr, vars, rules={}):
    ...
    for op in ["+", "-", "*", "/"]:
        for e in [
            x.strip()
            for x in markoutercomma(expr, comma=op).split("@" + op + "@")
        ]:
            if e in vars:
    ...
    for a in nout:
        if "depend" in vars[a]:
            for d in vars[a]["depend"]:
                if (
                    d in vars
                    and "depend" in vars[d]
                    and a in vars[d]["depend"]
                ):
                    errmess(
...
def character_backward_compatibility_hook(
    item, parents, result, *args, **kwargs
):
    ...
    def fix_usage(varname, value):
        ...
        value = re.sub(
            r"\b" + varname + r"\b\s*[\[]\s*0\s*[\]]", varname, value
        )
        return value
