...
class TestWhitespace:
    def setup_method(self):
        ...
        self.B = np.array([["abc", "123"], ["789", "xyz"]]).view(
            np.char.chararray
        )
...
class TestComparisons:
    def setup_method(self):
        self.A = np.array(
            [["abc", "abcc", "123"], ["789", "abc", "xyz"]]
        ).view(np.char.chararray)
        self.B = np.array(
            [["efg", "efg", "123  "], ["051", "efgg", "tuv"]]
        ).view(np.char.chararray)
...
class TestInformation:
    ...
    def test_find(self, dtype, encode):
        ...
        assert_array_equal(
            A.find(encode("a"), 0, 2), [[1, -1], [-1, -1], [-1, -1]]
        )
        assert_array_equal(
...
class TestMethods:
    ...
    def test_replace_count_and_size(self):
        a = np.array(["0123456789" * i for i in range(4)]).view(
            np.char.chararray
        )
        r1 = a.replace("5", "ABCDE")
        assert r1.dtype.itemsize == (3 * 10 + 3 * 4) * 4
        assert_array_equal(
            r1, np.array(["01234ABCDE6789" * i for i in range(4)])
        )
...
class TestOperations:
    def setup_method(self):
        ...
        self.B = np.array([["efg", "456"], ["051", "tuv"]]).view(
            np.char.chararray
        )
    ...
    def test_radd(self):
        QA = np.array([["qabc", "q123"], ["q789", "qxyz"]]).view(
            np.char.chararray
        )
...
class TestMethodsScalarValues:
    ...
    def test_join(self):
        seps = np.array(["-", "_"])
        assert_array_equal(
            np.char.join(seps, "hello"), ["h-e-l-l-o", "h_e_l_l_o"]
        )

    def test_partition(self):
        assert_equal(
            np.char.partition("This string", " "), ["This", " ", "string"]
        )
