...
UNARY_UFUNCS = [
    obj
    for obj in np._core.umath.__dict__.values()
...
class TestUfunc:
    ...
    def test_broadcast(self):
        ...
        assert_array_equal(
            np.vecdot(a, b), np.sum(a * b, axis=-1), err_msg=msg
        )
    ...
    def test_type_cast(self):
        ...
        assert_array_equal(
            np.vecdot(a, a), np.sum(a * a, axis=-1), err_msg=msg
        )
...
def test_ufunc_input_floatingpoint_error(bad_offset):
    value = 123
    arr = np.array(
        [value] * bad_offset + [np.nan] + [value] * int(1.5 * ncu.BUFSIZE)
    )
...
@pytest.mark.skipif(not HAS_REFCOUNT, reason="Python lacks refcounts")
@pytest.mark.parametrize(
    "offset", [0, ncu.BUFSIZE // 2, int(1.5 * ncu.BUFSIZE)]
)
def test_reduce_casterrors(offset):
    ...
    value = 123  # relies on python cache (leak-check will still find it)
    arr = np.array(
        [value] * offset + ["string"] + [value] * int(1.5 * ncu.BUFSIZE),
        dtype=object,
    )
