...
def test_sized_integer_casts(bitsize, signed):
    ...
    with pytest.raises(ValueError):
        np.array(
            ["1", np.nan, "3"], dtype=StringDType(na_object=np.nan)
        ).astype(idtype)
...
@pytest.mark.parametrize(
    ...
    [(DATETIME_INPUT, "M8[s]"), (TIMEDELTA_INPUT, "m8[s]")],
)
...
SUPPORTS_NULLS = (
    PASSES_THROUGH_NAN_NULLS + NULLS_ARE_FALSEY + NULLS_ALWAYS_ERROR
)
...
def call_func(func, args, array, sanitize=True):
    ...
    if args[0] is None:
        if sanitize:
            san_args = tuple(
                (
                    np.array(arg, dtype=array.dtype)
...
class TestImplementation:
    ...
    def is_on_heap(self, a):
        return self.get_flags(a) == (
            self.INITIALIZED | self.OUTSIDE_ARENA | self.LONG
        )
