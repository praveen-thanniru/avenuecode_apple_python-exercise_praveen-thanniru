...
def quote_args(args):
    ...
    warnings.warn(
        '"quote_args" is deprecated.', DeprecationWarning, stacklevel=2
    )
    ...
    if path is not None:
        ...
        for path in dirs:
            ...
            raise DistutilsError(
                "_numpyconfig.h not found in numpy include "
                "dirs %r" % (dirs,)
            )
            ...
            if "*" in n or "?" in n:
                ...
                if p2:
                    ...
                    if include_non_existing:
                        new_paths.append(n)
                    print(
                        "could not resolve pattern in %r: %r" % (local_path, n)
                    )
            else:
...
class Configuration:
    ...
    def _wildcard_get_subpackage(
        self, subpackage_name, parent_name, caller_level=1
    ):
    ...
    def get_subpackage(
        ...
        if not self.options["ignore_setup_xxx_py"]:
            if not os.path.isfile(setup_py):
                setup_py = njoin(
                    subpackage_path, "setup_%s.py" % (subpackage_name)
                )
        if not os.path.isfile(setup_py):
            if not self.options["assume_default_configuration"]:
                self.warn(
    ...
    def add_subpackage(
        self, subpackage_name, subpackage_path=None, standalone=False
    ):
        ...
        for config in config_list:
            ...
            self.info(
                "Appending %s configuration to %s" % (d.get("name"), self.name)
            )
            self.dict_append(**d)
...
def get_npy_pkg_dir():
    ...
    d = os.path.join(
        os.path.dirname(spec.origin), "_core", "lib", "npy-pkg-config"
    )
    return d
...
def msvc_version(compiler):
    ...
    if not compiler.compiler_type == "msvc":
        raise ValueError(
            "Compiler instance is not msvc (%s)" % compiler.compiler_type
        )
