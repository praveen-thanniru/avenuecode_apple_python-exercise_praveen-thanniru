...
class Exporter(object):
    ...
    def crawl_fig(self, fig):
        ...
        with self.renderer.draw_figure(
            fig=fig, props=utils.get_figure_properties(fig)
        ):
            for ax in fig.axes:
                self.crawl_ax(ax)
    ...
    def crawl_ax(self, ax):
        ...
        with self.renderer.draw_axes(
            ax=ax, props=utils.get_axes_properties(ax)
        ):
            ...
            for text, ttp in zip(
                ...
                if hasattr(text, "get_text") and text.get_text():
                    self.draw_text(
                        ax, text, force_trans=ax.transAxes, text_type=ttp
                    )
            ...
            try:
                ...
                if isinstance(child, matplotlib.patches.Patch):
                    ...
                    self.draw_collection(
                        ax, child, force_pathtrans=ax.transAxes
                    )
                else:
