...
class PlotlyRenderer(Renderer):
    ...
    def draw_bar(self, coll):
        ...
        vertical = (
            abs(sum(widths[0] - widths[iii] for iii in range(len(widths))))
            < tol
        )
        horizontal = (
            abs(sum(heights[0] - heights[iii] for iii in range(len(heights))))
            < tol
        )
        ...
        for single_mode in mode.split("+"):
            ...
            if single_mode == "markers" and props.get("markerstyle"):
                ...
                path = " ".join(
                    [
                        f"{a} {t[0]},{t[1]}"
                        for a, t in zip(markerpath[1], markerpath[0])
                    ]
                )
    ...
    def draw_path_collection(self, **props):
        ...
        if props["offset_coordinates"] == "data":
            ...
            self.msg += (
                "    Path collection not linked to 'data', " "not drawing\n"
            )
            warnings.warn(
    ...
    def draw_text(self, **props):
        ...
        if props["text_type"] == "xlabel":
            ...
            if props["coordinates"] != "data":
                self.msg += (
                    "        Text object isn't linked to 'data' "
                    "coordinates\n"
                )
                x_px, y_px = (
                    props["mplobj"]
    ...
    def draw_title(self, **props):
        ...
        if len(self.mpl_fig.axes) > 1:
            self.msg += (
                "          More than one subplot, adding title as "
                "annotation\n"
            )
            x_px, y_px = (
            ...
            self.msg += (
                "          Only one subplot found, adding as a "
                "plotly title\n"
            )
