...
def _create_us_counties_df(st_to_state_name_dict, state_to_st_dict):
    ...
    df_shape_pre2010["FIPS"] = (
        df_shape_pre2010["STATE"] + df_shape_pre2010["COUNTY"]
    )
    ...
    singlerow = pd.DataFrame(
        [
            [
                ...
                "SD",
                df_shape_pre2010[df_shape_pre2010["FIPS"] == f][
                    "geometry"
                ].iloc[0],
                df_shape_pre2010[df_shape_pre2010["FIPS"] == f]["FIPS"].iloc[
                    0
                ],
                "46",
                "Shannon",
    ...
    gdf_statefp = gdf_reduced.merge(
        df_state[["STATEFP", "STATE_NAME"]], on="STATEFP"
    )
...
def _intervals_as_labels(
    array_of_intervals, round_legend_values, exponent_format
):
    """
...
def _calculations(
    ...
    if fips_polygon_map[f].type == "Polygon":
        ...
        x_c = [
            poly.centroid.xy[0].tolist() for poly in fips_polygon_map[f].geoms
        ]
        y_c = [
            poly.centroid.xy[1].tolist() for poly in fips_polygon_map[f].geoms
        ]
...
"""
    ...
    df, df_state = _create_us_counties_df(
        st_to_state_name_dict, state_to_st_dict
    )
    ...
    if binning_endpoints:
        ...
        LEVELS = _intervals_as_labels(
            intervals, round_legend_values, exponent_format
        )
    else:
        if not order:
    ...
    DEFAULT_LAYOUT = dict(
        ...
        dragmode="select",
        legend=dict(
            traceorder="reversed", xanchor="right", yanchor="top", x=1, y=1
        ),
        annotations=[],
    )
