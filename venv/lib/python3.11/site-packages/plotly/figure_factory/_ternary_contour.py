...
def _ternary_layout(
    ...
    return dict(
        ...
        ternary=dict(
            sum=1,
            aaxis=dict(
                title=dict(text=pole_labels[0]),
                min=0.01,
                linewidth=2,
                ticks="outside",
            ),
            baxis=dict(
                title=dict(text=pole_labels[1]),
                min=0.01,
                linewidth=2,
                ticks="outside",
            ),
            caxis=dict(
                title=dict(text=pole_labels[2]),
                min=0.01,
                linewidth=2,
                ticks="outside",
            ),
...
def _ilr_transform(barycentric):
    ...
    x_1 = (
        1.0
        / np.sqrt(6)
        * np.log(barycentric[0] * barycentric[1] / barycentric[2] ** 2)
    )
...
def _prepare_barycentric_coord(b_coords):
    ...
    if b_coords.shape[0] not in (2, 3):
        raise ValueError(
            "A point should have  2 (a, b) or 3 (a, b, c)"
            "barycentric coordinates"
        )
    if (
        (len(b_coords) == 3)
...
def _extract_contours(im, values, colors):
    ...
    for i, val in enumerate(values):
        ...
        all_areas1.extend(
            [
                _polygon_area(contour.T[1], contour.T[0])
...
def create_ternary_contour(
    ...
    grid_z, gr_x, gr_y = _compute_grid(
        coordinates, values, interp_mode=interp_mode
    )
