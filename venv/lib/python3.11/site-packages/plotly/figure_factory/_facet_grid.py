...
def _add_shapes_to_fig(
    fig, annot_rect_color, flipped_rows=False, flipped_cols=False
):
    shapes_list = []
...
def _facet_grid_color_categorical(
    ...
    if not facet_row and not facet_col:
        ...
        for row_count, x_val in enumerate(row_values):
            ...
            label = _return_label(
                row_values[row_count], facet_row_labels, facet_row
            )
            annotations.append(
                _annotation_dict(
                    label,
...
def _facet_grid_color_numerical(
    ...
    if not facet_row and not facet_col:
        trace = dict(
            type=trace_type,
            marker=dict(
                color=df[color_name], colorscale=colormap, showscale=True
            ),
            **kwargs_trace,
        )
        if x:
        ...
        for row_count, x_val in enumerate(row_values):
            for col_count, y_val in enumerate(col_values):
                ...
                if group.values.tolist() != [[None, None, None]]:
                    ...
                    trace = dict(
                        type=trace_type, showlegend=False, **kwargs_trace
                    )
        ...
        for row_count, x_val in enumerate(row_values):
            for col_count, y_val in enumerate(col_values):
                ...
                trace = dict(
                    type=trace_type,
                    marker=dict(
                        color=marker_color, line=kwargs_marker["line"]
                    ),
                    **kwargs_trace,
                )
                if x:
