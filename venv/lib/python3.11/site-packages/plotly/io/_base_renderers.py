...
class BaseRenderer(object):
    ...
    def __repr__(self):
        ...
        return "{cls}({attrs})\n{doc}".format(
            ...
            attrs=", ".join(
                "{}={!r}".format(k, self.__dict__[k]) for k in init_args
            ),
            doc=self.__doc__,
        )
...
""".format(
    ...
    def build_filename(self):
        ...
        try:
            cell_number = (
                list(ip.history_manager.get_tail(1))[0][1] + 1 if ip else 0
            )
        except Exception:
            cell_number = 0
...
def open_html_in_browser(html, using=None, new=0, autoraise=True):
    ...
    if using is None:
        ...
        if browser is None:
            raise ValueError(
                "Can't locate a browser with key in " + str(using)
            )
    ...
    def displayHTML(self):
        ...
        if self._displayHTML is None:
            for frame in inspect.getouterframes(inspect.currentframe()):
                ...
                if all(
                    v in global_names
                    for v in ["displayHTML", "display", "spark"]
                ):
