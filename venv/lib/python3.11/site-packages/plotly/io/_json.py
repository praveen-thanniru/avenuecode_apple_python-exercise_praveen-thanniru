...
def to_json_plotly(plotly_object, pretty=False, engine=None):
    ...
    if engine == "json":
        ...
        return _safe(
            json.dumps(plotly_object, cls=PlotlyJSONEncoder, **opts),
            _swap_json,
        )
    elif engine == "orjson":
        ...
        try:
            return _safe(
                orjson.dumps(plotly_object, option=opts).decode("utf8"),
                _swap_orjson,
            )
        except TypeError:
            pass
        ...
        return _safe(
            orjson.dumps(cleaned, option=opts).decode("utf8"), _swap_orjson
        )
