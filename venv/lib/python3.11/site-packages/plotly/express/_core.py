...
TraceSpec = namedtuple(
    "TraceSpec", ["constructor", "attrs", "trace_patch", "marginal"]
)
...
def get_decorated_label(args, column, role):
    ...
    if "histfunc" in args and (
        ...
        or (
            role == "x"
            and "orientation" in args
            and args["orientation"] == "h"
        )
        or (
            role == "y"
            and "orientation" in args
            and args["orientation"] == "v"
        )
    ):
        ...
        if attr_name == "dimensions":
            ...
            if attr_name == "size":
                ...
                if (
                    ...
                    mapping_labels[get_label(args, args["y"])] = (
                        "%{y} <b>(trend)</b>"
                    )
                ...
                if trace_spec.constructor not in [
                    ...
                    for col in attr_value:
                        ...
                        mapping_labels[attr_label_col] = (
                            "%%{customdata[%d]}" % (position)
                        )
                ...
                if trace_spec.constructor in [
                    ...
                    if args.get("color_is_continuous"):
                        trace_patch["marker"]["colors"] = trace_data[
                            attr_value
                        ]
                        ...
                        for cat in trace_data[attr_value]:
                            if mapping.get(cat) is None:
                                mapping[cat] = args["color_discrete_sequence"][
                                    len(mapping)
                                    % len(args["color_discrete_sequence"])
                                ]
                            trace_patch["marker"]["colors"].append(
                                mapping[cat]
                            )
                else:
                    colorable = "marker"
            ...
            trace_spec.constructor == go.Histogram and attr_name in ["x", "y"]
        ) or (
...
def configure_cartesian_marginal_axes(args, fig, orders):
    ...
    if args["marginal_x"]:
        ...
        if args["template"].layout.yaxis.showgrid is None:
            fig.update_yaxes(
                showgrid=args["marginal_x"] == "histogram", row=nrows
            )
    ...
    if args["marginal_y"]:
        ...
        if args["template"].layout.xaxis.showgrid is None:
            fig.update_xaxes(
                showgrid=args["marginal_y"] == "histogram", col=ncols
            )
        if args["template"].layout.yaxis.showgrid is None:
...
def configure_animation_controls(args, constructor, fig):
    ...
    if (
        "animation_frame" in args
        and args["animation_frame"]
        and len(fig.frames) > 1
    ):
...
def make_trace_spec(args, constructor, attrs, trace_patch):
    ...
    if (
        args.get("trendline")
        and args.get("trendline_scope", "trace") == "trace"
    ):
...
def apply_default_cascade(args):
    ...
    if "color_discrete_sequence" in args:
        if (
            args["color_discrete_sequence"] is None
            and args["template"].layout.colorway
        ):
    ...
    if "symbol_sequence" in args:
        if args["symbol_sequence"] is None and args["template"].data.scatter:
            args["symbol_sequence"] = [
                ...
                for scatter in args["template"].data.scatter
            ]
    ...
    if "line_dash_sequence" in args:
        if (
            args["line_dash_sequence"] is None
            and args["template"].data.scatter
        ):
            args["line_dash_sequence"] = [
                scatter.line.dash for scatter in args["template"].data.scatter
            ]
        if not args["line_dash_sequence"] or not any(
            args["line_dash_sequence"]
        ):
            args["line_dash_sequence"] = [
                "solid",
    ...
    if "pattern_shape_sequence" in args:
        if (
            args["pattern_shape_sequence"] is None
            and args["template"].data.bar
        ):
...
def _check_name_not_reserved(field_name, reserved_names):
    if field_name not in reserved_names:
        ...
        raise NameError(
            ...
            "A column or index with name '%s' is ambiguous."
...
def process_args_into_dataframe(args, wide_mode, var_name, value_name):
    ...
    df_output.update(
        {
            col_name: to_unindexed_series(range(length), col_name)
    ...
    df_output.update(
        {
            ...
            col_name: to_unindexed_series(
                [constants[col_name]] * length, col_name
            )
...
def build_dataframe(args, constructor):
    ...
    if needs_interchanging:
        try:
            if wide_mode or not hasattr(
                args["data_frame"], "select_columns_by_name"
            ):
                ...
                necessary_columns = {
                    ...
                    if isinstance(i, str) and i in columns
                }
                for field in args:
                    if args[field] is not None and field in array_attrables:
                        necessary_columns.update(
            ...
            if df_provided:
                ...
                args["wide_cross"] = Range(
                    label=_escape_col_name(
                        df_input, "index", [var_name, value_name]
                    )
                )
    ...
    if wide_mode:
        ...
        wide_value_vars = [
            c for c in args["wide_variable"] if c not in wide_id_vars
        ]
    ...
    for i, level in enumerate(path):
        ...
        if i < len(path) - 1:
            ...
            while j < len(path):
                ...
                df_tree["id"] = (
                    dfg[path[j]].copy().astype(str) + "/" + df_tree["id"]
                )
                j += 1
...
def process_dataframe_pie(args, trace_patch):
    ...
    order = [
        x
        for x in OrderedDict.fromkeys(list(order_in) + uniques)
        if x in uniques
    ]
...
def infer_config(args, constructor, trace_patch, layout_patch):
    ...
    if "size" in args and args["size"]:
        sizeref = (
            args["data_frame"][args["size"]].max() / args["size_max"] ** 2
        )
    ...
    if "color" in args:
        if "color_continuous_scale" in args:
            if "color_discrete_sequence" not in args:
                ...
                if args["color"] and _is_continuous(
                    args["data_frame"], args["color"]
                ):
                    attrs.append("color")
    ...
    if args.get("text_auto", False) is not False:
        ...
        if args["text_auto"] is True:
            ...
            trace_patch["texttemplate"] = (
                "%{" + letter + ":" + args["text_auto"] + "}"
            )
    ...
    if "geojson" in args:
        ...
        trace_patch["geojson"] = (
            args["geojson"]
            if not hasattr(
                args["geojson"], "__geo_interface__"
            )  # for geopandas
    ...
    if "marginal" in args:
        ...
        other_position = (
            "marginal_x" if args["orientation"] == "h" else "marginal_y"
        )
    ...
    if (
        args.get("facet_col") is not None
        and args.get("marginal_y") is not None
    ):
        args["marginal_y"] = None

    if (
        args.get("facet_row") is not None
        and args.get("marginal_x") is not None
    ):
...
def get_groups_and_orders(args, grouper):
    ...
    """
    orders = (
        {} if "category_orders" not in args else args["category_orders"].copy()
    )
    ...
    if len(single_group_name) == len(grouper):
        ...
        for i, col in reversed(list(enumerate(required_grouper))):
            sorted_group_names = sorted(
                sorted_group_names,
                key=lambda g: (
                    orders[col].index(g[i]) if g[i] in orders[col] else -1
                ),
            )
...
def make_figure(args, constructor, trace_patch=None, layout_patch=None):
    ...
    if (
        constructor in [go.Treemap, go.Sunburst, go.Icicle]
        ...
        if m.grouper not in orders:
            ...
            for val in sorted_values:
                if val not in m.val_map:  # always False if it's an IdentityMap
                    m.val_map[val] = m.sequence[
                        len(m.val_map) % len(m.sequence)
                    ]
            ...
            for i, m in enumerate(grouped_mappings):
                ...
                try:
                    ...
                    if (
                        ...
                        in [go.Choropleth, go.Choroplethmapbox]
                        and m.variable == "color"
                    ):
                        trace.update(
                            z=[1] * len(group),
                ...
                if m.facet == "row":
                    ...
                    if (
                        args.get("marginal_x")
                        is not None  # there is a marginal
    ...
    if len(frame_list) > 1:
        frame_list = sorted(
            frame_list,
            key=lambda f: orders[args["animation_frame"]].index(f["name"]),
        )
    ...
    if (
        args.get("trendline")
        and args.get("trendline_scope", "trace") == "overall"
    ):
...
def init_figure(
    ...
    # Build subplot specs
    specs = [
        [dict(type=subplot_type or "domain")] * ncols for _ in range(nrows)
    ]
    ...
    if facet_col_wrap:
        ...
        for i in range(nrows):
            for j in range(ncols):
                subplot_labels[i * ncols + j] = col_labels[
                    (nrows - 1 - i) * ncols + j
                ]
