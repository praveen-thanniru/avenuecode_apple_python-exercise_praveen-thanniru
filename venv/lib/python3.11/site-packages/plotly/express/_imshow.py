...
def imshow(
    ...
    if isinstance(img, pd.DataFrame):
        if binary_string:
            raise ValueError(
                "Binary strings cannot be used with pandas arrays"
            )
        is_dataframe = True
    else:
        is_dataframe = False
    ...
    if contrast_rescaling is None:
        contrast_rescaling = (
            "minmax" if img.ndim == (2 + slice_dimensions) else "infer"
        )
    ...
    if facet_col is not None:
        slice_label = (
            "facet_col"
            if labels.get("facet_col") is None
            else labels["facet_col"]
        )
    ...
    for index, trace in enumerate(traces):
        if (facet_col and index < nrows * ncols) or index == 0:
            fig.add_trace(
                trace, row=nrows - index // ncols, col=index % ncols + 1
            )
    if animation_frame is not None:
