...
class BaseFigureWidget(BaseFigure, widgets.DOMWidget):
    ...
    _py2js_deleteTraces = Dict(allow_none=True).tag(
        sync=True, **custom_serializers
    )
    _py2js_moveTraces = Dict(allow_none=True).tag(
        sync=True, **custom_serializers
    )
    ...
    _py2js_removeTraceProps = Dict(allow_none=True).tag(
        sync=True, **custom_serializers
    )
    ...
    # See JSDoc comments in the FigureModel class in js/src/Figure.js for
    # detailed descriptions of the messages.
    _js2py_traceDeltas = Dict(allow_none=True).tag(
        sync=True, **custom_serializers
    )
    _js2py_layoutDelta = Dict(allow_none=True).tag(
        sync=True, **custom_serializers
    )
    _js2py_restyle = Dict(allow_none=True).tag(sync=True, **custom_serializers)
    _js2py_relayout = Dict(allow_none=True).tag(
        sync=True, **custom_serializers
    )
    _js2py_update = Dict(allow_none=True).tag(sync=True, **custom_serializers)
    _js2py_pointsCallback = Dict(allow_none=True).tag(
        sync=True, **custom_serializers
    )
    ...
    def _send_restyle_msg(
        self, restyle_data, trace_indexes=None, source_view_id=None
    ):
        """
    ...
    def _handler_js2py_relayout(self, change):
        ...
        # Perform relayout
        # ----------------
        self.plotly_relayout(
            relayout_data=relayout_data, source_view_id=source_view_id
        )
    ...
    def _handler_js2py_pointsCallback(self, change):
        ...
        if callback_data.get("selector", None):
            ...
            if selector_type == "box":
                ...
                raise ValueError(
                    "Unsupported selector type: %s" % selector_type
                )
        else:
            selector = None
    ...
    def _transform_data(
        ...
        if isinstance(to_data, dict):
            ...
            for from_prop, from_val in from_data.items():
                ...
                if isinstance(from_val, dict) or BaseFigure._is_dict_list(
                    from_val
                ):
                    ...
                    if from_prop not in to_data:
                        to_data[from_prop] = (
                            {} if isinstance(from_val, dict) else []
                        )
        ...
        # -----------
            ...
            for i, from_val in enumerate(from_data):
                ...
                if input_val is not None and (
                    isinstance(from_val, dict)
                    or BaseFigure._is_dict_list(from_val)
                ):
