...
def _get_initial_max_subplot_ids():
    max_subplot_ids = {
        subplot_type: 0 for subplot_type in _single_subplot_types
    }
...
make_subplots: {k}
    ...
    if specs is None:
        ...
        and all(
            all(v is None or isinstance(v, dict) for v in row) for row in specs
        )
    ):
        raise ValueError(
            """
...
""".format(
    ...
    if insets is None or insets is False:
        ...
        isinstance(insets, (list, tuple))
        and all(isinstance(v, dict) for v in insets)
    ):
        raise ValueError(
            """
...
The resulting plot would have {dimsize} {dimname} ({dimvarname}={dimsize}).""".format(
    ...
    _check_hv_spacing(
        cols, horizontal_spacing, "Horizontal", "cols", "columns"
    )
    ...
    if column_widths is None:
        ...
        isinstance(column_widths, (list, tuple)) and len(column_widths) == cols
    ):
...
length {cols}.
    ...
    if row_heights is None:
        ...
        for h in row_heights:
            heights.append(
                (1.0 - vertical_spacing * (rows - 1)) * (h / cum_sum)
            )
...
The row_titles argument to make_subplots must be a list or tuple
    ...
    for r, spec_row in enumerate(specs):
        for c, spec in enumerate(spec_row):
            ...
            if c_spanned >= cols:
                raise Exception(
                    "Some 'colspan' value is too large for "
                    "this subplot grid."
                )
            if r_spanned >= rows:
                raise Exception(
                    "Some 'rowspan' value is too large for "
                    "this subplot grid."
                )
        ...
        for c in range(cols):
            for ri, r in enumerate(rows_iter):
                ...
                if x_or_y == "y":
                    ...
                    ok_to_remove_label = (
                        ri > 0 if row_dir > 0 else r < rows - 1
                    )
...
def _validate_coerce_subplot_type(subplot_type):
    ...
    if subplot_type is None:
        raise ValueError(
            "Unsupported subplot type: {}".format(repr(orig_subplot_type))
        )
    else:
        return subplot_type
...
def _init_subplot(
    ...
    if subplot_type == "xy":
        ...
        raise ValueError(
            "Unsupported subplot type: {}".format(repr(subplot_type))
        )
...
def _build_subplot_title_annotations(
    ...
    if title_edge == "top":
        ...
        raise ValueError(
            "Invalid annotation edge '{edge}'".format(edge=title_edge)
        )
...
def _build_grid_str(specs, grid_ref, insets, insets_ref, row_seq):
    ...
    def _get_cell_str(r, c, subplot_refs):
        layout_keys = sorted(
            {k for ref in subplot_refs for k in ref.layout_keys}
        )
...
def _get_subplot_ref_for_trace(trace):
    if "domain" in trace:
        return SubplotRef(
            ...
            layout_keys=(),
            trace_kwargs={
                "domain": {"x": trace.domain.x, "y": trace.domain.y}
            },
        )
