...
def _str_to_dict_path_full(key_path_str):
    ...
    if len(key_path_str):
        ...
        key_elem_pairs = list(
            filter(lambda t: len(t[1]), enumerate(key_path2d))
        )
...
%s"""
    ...
    def __setitem__(self, prop, value):
        ...
        # e.g. ('foo', 1)
            ...
            err = _check_path_in_prop_tree(
                self, orig_prop, error_cast=ValueError
            )
            if err is not None:
                raise err
            res = self
    ...
    def __getitem__(self, prop):
        ...
        # e.g. ('foo', 1)
            ...
            err = _check_path_in_prop_tree(
                self, orig_prop, error_cast=PlotlyKeyError
            )
            if err is not None:
                raise err
            res = self
    ...
    def data(self, new_data):
        ...
        for trace in new_data:
            if not isinstance(trace, BaseTraceType):
                err_msg = (
                    err_header
                    + "    Received element value of type {typ}".format(
                        typ=type(trace)
                    )
                )
                raise ValueError(err_msg)
    ...
    def _select_layout_subplots_by_prefix(
        ...
        layout_keys_filters = [
            ...
            lambda k: (
                secondary_y is None
                or container_to_row_col.get(k, (None, None, None))[2]
                == secondary_y
            ),
        ]
        layout_keys = reduce(
    ...
    def _select_annotations_like(
        ...
        return _generator(
            self._filter_by_selector(self.layout[prop], funcs, selector)
        )
...
is of type {subplot_type}.""".format(
    ...
    because subplot does not have a secondary y-axis""".format(
                ...
                xref, yref = xaxis.replace("axis", ""), yaxis.replace(
                    "axis", ""
                )
            else:
                yref = new_obj.yref
            ...
            def _add_domain(ax_letter, new_axref):
                axref = ax_letter + "ref"
                if (
                    axref in new_obj._props.keys()
                    and "domain" in new_obj[axref]
                ):
                    new_axref += " domain"
                return new_axref

            xref, yref = map(
                lambda t: _add_domain(*t), zip(["x", "y"], [xref, yref])
            )
...
""".format(
    ...
    def _set_in(d, key_path_str, v):
        ...
        for kp, key_path_el in enumerate(key_path[:-1]):
            ...
            if isinstance(val_parent, list) and isinstance(key_path_el, int):
                ...
                isinstance(val_parent, dict) and key_path_el not in val_parent
            ):
        ...
        # Check whether we can remove key from parent
            ...
            if isinstance(val_parent, dict):
                ...
                if isinstance(last_key, int) and 0 <= last_key < len(
                    val_parent
                ):
    ...
    Cannot remove element of type {typ} at location {raw_key}""".format(
        ...
        # Check whether parent should be updated
            ...
            if isinstance(val_parent, dict):
                if (
                    last_key not in val_parent
                    or not BasePlotlyType._vals_equal(val_parent[last_key], v)
                ):
    ...
    def add_traces(
        ...
        # Apply rows / cols
        if rows is not None:
            for trace, row, col, secondary_y in zip(
                data, rows, cols, secondary_ys
            ):
...
""".format(
    ...
    def _send_restyle_msg(
        self, style, trace_indexes=None, source_view_id=None
    ):
        pass
    ...
    def _perform_batch_animate(self, animation_opts):
        ...
        (
            ...
            restyle_data, relayout_data, trace_indexes
        )
    ...
    def _to_ordered_dict(d, skip_uid=False):
        ...
        if isinstance(d, dict):
            ...
            result = [
                BaseFigure._to_ordered_dict(el, skip_uid=skip_uid) for el in d
            ]
        else:
            result = d
    ...
    def to_ordered_dict(self, skip_uid=True):
        ...
        # Handle data
        # -----------
        result["data"] = BaseFigure._to_ordered_dict(
            self._data, skip_uid=skip_uid
        )
    ...
    def _perform_update(plotly_obj, update_obj, overwrite=False):
        ...
        if update_obj is None:
            ...
            for key in update_obj:
                ...
                if key not in plotly_obj and isinstance(
                    plotly_obj, BaseLayoutType
                ):
            ...
            for key in update_obj:
                ...
                if isinstance(validator, CompoundValidator) and isinstance(
                    val, dict
                ):
            ...
            raise ValueError(
                "Unexpected plotly object with type {typ}".format(
                    typ=type(plotly_obj)
                )
            )
    ...
    def _process_multiple_axis_spanning_shapes(
        ...
        **kwargs,
        ...
        shape_kwargs, annotation_kwargs = (
            shapeannotation.split_dict_by_key_prefix(kwargs, "annotation_")
        )
            ...
            new_layout_objs = tuple(
                filter(
                    ...
                    [
                        ...
                        for n in range(
                            n_layout_objs_before, n_layout_objs_after
                        )
                    ],
                )
            )
            self.layout[layout_obj] = (
                self.layout[layout_obj][:n_layout_objs_before]
                + new_layout_objs
            )
...
class BasePlotlyType(object):
    ...
    def _get_child_props(self, child):
        ...
        if self._props is None:
            ...
            # ### Invalid child ###
                ...
                raise ValueError(
                    "Invalid child with name: %s" % child.plotly_name
                )
    ...
    def __getitem__(self, prop):
        ...
        if len(prop) == 1:
            ...
            if prop not in self._valid_props:
                self._raise_on_invalid_property_error(
                    _error_to_raise=PlotlyKeyError
                )(prop)
    ...
    def __setattr__(self, prop, value):
        ...
        if (
            prop.startswith("_")
            or hasattr(self, prop)
            or prop in self._valid_props
        ):
            # Let known properties and private properties through
        ...
        def _ret(*args):
            ...
            if invalid_props:
                ...
                if guessed_prop is not None:
                    guessed_prop_suggestion = 'Did you mean "%s"?' % (
                        guessed_prop,
                    )
    ...
    def _vals_equal(v1, v2):
        ...
        if np is not None and (
            ...
            return (
                ...
                and len(v1) == len(v2)
                and all(
                    BasePlotlyType._vals_equal(e1, e2)
...
class BaseLayoutType(BaseLayoutHierarchyType):
    ...
    def _process_kwargs(self, **kwargs):
        ...
        subplot_kwargs = {
            k: v for k, v in kwargs.items() if self._subplot_re_match(k)
        }
