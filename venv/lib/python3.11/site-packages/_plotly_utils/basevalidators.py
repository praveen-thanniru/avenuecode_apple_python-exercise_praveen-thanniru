    ...
    if pd and isinstance(v, (pd.Series, pd.Index)):
        ...
        if dtype.kind in numeric_kinds:
            ...
            with warnings.catch_warnings():
                ...
                # https://github.com/pandas-dev/pandas/pull/52459
                v = [
                    np.array(row.dt.to_pydatetime()).tolist()
...
class SrcValidator(BaseValidator):
    ...
    def validate_coerce(self, v):
        if v is None:
            ...
            pass
        elif self.chart_studio and isinstance(
            v, self.chart_studio.grid_objs.Column
        ):
    ...
    def validate_coerce(self, v):
        if v is None:
            ...
            if is_homogeneous_array(v):
                ...
                if not self.strict:
                    v = [
                        StringValidator.to_str_or_unicode_or_none(e) for e in v
                    ]
    ...
    def validate_coerce(self, v, should_raise=True):
        if v is None:
            ...
            if self.numbers_allowed() and v.dtype.kind in ["u", "i", "f"]:
                ...
                validated_v = [
                    self.validate_coerce(e, should_raise=False) for e in v
                ]
    ...
    def validate_coerce(self, v):
        ...
        if v is None:
            ...
            if v_lower in self.named_colorscales:
                ...
                v_lower.endswith("_r")
                and v_lower[:-2] in self.named_colorscales
            ):
            ...
            if isinstance(v[0], str):
                invalid_els = [
                    e
                    for e in v
                    if ColorValidator.perform_validate_coerce(e) is None
                ]
                ...
                if len(invalid_els) == 0:
                    ...
                    v = [
                        [e[0], ColorValidator.perform_validate_coerce(e[1])]
                        for e in v
                    ]
...
class SubplotidValidator(BaseValidator):
    ...
    def __init__(
        self, plotly_name, parent_name, dflt=None, regex=None, **kwargs
    ):
...
class AnyValidator(BaseValidator):
    ...
    def __init__(
        self, plotly_name, parent_name, values=None, array_ok=False, **kwargs
    ):
...
class InfoArrayValidator(BaseValidator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        info_array_items = (
            self.items if isinstance(self.items, list) else [self.items]
        )
...
class CompoundValidator(BaseValidator):
    def __init__(
        self, plotly_name, parent_name, data_class_str, data_docs, **kwargs
    ):
    ...
    def validate_coerce(self, v, skip_invalid=False, _validate=True):
        if v is None:
            ...
            v = self.data_class(
                v, skip_invalid=skip_invalid, _validate=_validate
            )
        ...
        if v is None:
            ...
            for v_el in v:
                if isinstance(v_el, self.data_class):
                    ...
                    res.append(
                        self.data_class(v_el, skip_invalid=skip_invalid)
                    )
                else:
                    if skip_invalid:
...
{trace_types}
    ...
    def get_trace_class(self, trace_name):
        ...
        if trace_name not in self._class_map:
            ...
            self._class_map[trace_name] = getattr(
                trace_module, trace_class_name
            )
...
class BaseTemplateValidator(CompoundValidator):
    ...
    def validate_coerce(self, v, skip_invalid=False):
        ...
        if (
            v == {}
            or isinstance(v, self.data_class)
            and v.to_plotly_json() == {}
        ):
