def _swatches(module_names, module_contents, template=None):
    ...
    sequences = [
        ...
        if not (
            k.startswith("_") or k.startswith("swatches") or k.endswith("_r")
        )
    ]
    ...
    return go.Figure(
        ...
        layout=dict(
            ...
            showlegend=False,
            xaxis=dict(
                range=[-0.02, 1.02], showticklabels=False, showgrid=False
            ),
    ...
    return go.Figure(
        data=[
            go.Bar(
                ...
                marker=dict(
                    color=list(range(n)), colorscale=name, line_width=0
                ),
...
def _swatches_cyclical(module_names, module_contents, template=None):
    ...
    fig.update_traces(
        width=5.2, marker_line_width=0, base=0.5, showlegend=False
    )
